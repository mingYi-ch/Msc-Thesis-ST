...
expr_comparison_scatter <- as_tibble(colData(spe)) %>%
  ...
  ggplot(aes(x = dist.cluster, y = expr, fill = dist.cluster)) +
    ...
    scale_y_continuous(limits = c(0, 15), expand = expansion(add = 0)) +
...
expr_comparison_pl <- comparison_data %>%
  ggplot(aes(x = dist.cluster, y = expr)) +
    ...
    # geom_smooth(aes(color = origin, x = as.integer(dist.cluster)), method = 'gam', formula = y ~ s(x, bs = "cs", k = 5), se = FALSE, linewidth = 2) +
    geom_errorbar(aes(ymin = lower, ymax = upper, color = origin), width = 0.8) +
...
```{r}
filename <- paste0("correlation plot of dists",  ".png")
ggsave(filename = file.path(res.dir, "plots", filename), plot = expr_comparison_pl, device = "png", width = 10, height = 8, dpi = 500)
```
...
```{r}
plot <- expr_comparison_pl / rel_plt /rel_plt_2
...
# Define a function to calculate the statistic of interest (mean in this case)
statistic_function <- function(data, indices) {
  return(mean(data[indices]))
}

# Perform bootstrap
boot_results <- boot(data, statistic_function, R = 1000)

# Calculate the confidence intervals
ci <- boot.ci(boot_results, conf = c(0.95), type = c('perc'))
ci$percent
# Print the results
print(ci)
