...
expr_comparison_pl <- tibble(expr = comparison_data) %>%
  ...
  ggplot(aes(x = avg.dist, y = expr, text = hover_text)) +
    ...
    guides(
        ...
        color = guide_legend(override.aes = list(linetype = 1, linewidth = 2, size = size.axis.text / 2)))  # Set legend as lines
...
for (c in seq(4)) {
  your_data <- markers_cluster %>% filter(cluster == as.character(c))
  ...
  datatable(your_data, options = list(pageLength = 10, autoWidth = TRUE),  escape = FALSE) %>% print()
}
