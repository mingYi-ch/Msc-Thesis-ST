...
umap_decr <- as_tibble(umap) %>%
  ...
  labs(subtitle = glue("Cluster of centroids - ALL - in UMAP"),
       x = "UMAP 1",
       y = "UMAP 2")
...
genes.all <- rownames(centroids.subset)

for (k in seq(K)) {
  filename <-  glue("{k}_plots.pdf")
  filepath <- file.path(res.dir, "plots", filename)
  
  # Open a PDF device to save the plots
  pdf(filepath, width = 10, height = 8)
  genes <- genes.all[which(clusters == as.character(k))]
...
extract_median_both <- function(res, genes.syn = genes.all) {
  medians <- as_tibble(res_count) %>% 
    filter(symbol %in% genes.syn) %>% 
    group_by(origin, dist.cluster, symbol) %>% 
    summarize(median_expr = median(expr, na.rm = TRUE), .groups = "drop") 
  return(medians)
}

extract_median_both(boot.res[[1]])

gene_medians_both <- lapply(boot.res, extract_median_both) %>% do.call(rbind, .)

head(gene_medians_both)
...
cal.logFC <-  function(gexp) {
  logFC <- as_tibble(gexp) %>% 
    ...
    summarize(logFC = log2(inside/outside), .groups = "drop") 
  return(logFC)
}
...
logFC_quantile <- as_tibble(logFC) %>% 
  group_by(symbol) %>% 
  summarize(logFC = quantile(logFC, probs = quantile), .groups = "drop") 
...
if (rerun.boot) {
  res_outside <- perform_bootstrapping("outside", genes_of_interest, id_target)
