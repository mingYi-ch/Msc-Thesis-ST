...
plots_marker_sp_bar <- as_tibble(gexp.target) %>% 
  ggplot(aes(x = value)) +
    ...
    theme_minimal() +
    labs(title = paste0("Frequency of ", marker.sel, " in spatial data"),
         x = "UMI",
         y = "Frequency")

filename <- paste0("Bar plot of Frequency of ", marker.sel, ".svg")
ggsave(filename = file.path(res.dir, "plots", filename), plot = plots_marker_sp_bar, device = "svg", width = 10, height = 8)

filename <- paste0("Bar plot of Frequency of ", marker.sel, ".png")
ggsave(filename = file.path(res.dir, "plots", filename), plot = plots_marker_sp_bar, device = "png", width = 10, height = 8, dpi = 500)
...
# Perform bootstrap with R = 1000
set.seed(123)  # For reproducibility
boot_results <- boot(data = data, statistic = mean_function, R = 1000)
