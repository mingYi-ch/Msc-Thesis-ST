...
marker.sel <- "VWF"
is.ec.spot.VWF <- qread(file.path(res.dir, "deconvolution/ECs_manually_VWF.qs"))
...
marker.sel <- "PECAM1"
is.ec.spot.PECAM1 <- qread(file.path(res.dir, "deconvolution/ECs_manually_PECAM1.qs"))
is.ec.spot <- is.ec.spot.PECAM1
...
if (re.run) {
  min.dists <- get.min.dists(dists.mat, weights, normalize = T)
...
# spe$dist.cluster <- bin.dists3(spe$min.dists, label_fmt = "%.f", min_spots = 100)
